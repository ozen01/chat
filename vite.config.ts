...
// In-memory storage for Socket.io
interface Message {
  id: string;
  username: string;
  text: string;
...
function socketioPlugin(): Plugin {
  ...
  return {
    ...
    configureServer(server) {
      ...
      // Attach Express to Vite's middleware
      server.middlewares.use(app);

      // Return a hook that runs after the HTTP server is created
      return () => {
        // Get Vite's HTTP server and attach Socket.io to it
